...
TEST(message_test_message, can_serialize_const_message_view)
{
  std::default_random_engine random_engine;
  ...
  kd::message_body const body_out{ kd::find_peer_request_body{
      .peer_to_find{ random_engine },
  } };
  kd::const_message_view message_out{ .header = header_out, .body = body_out };
  ...
  ASSERT_EQ(message_out.header, message_in.header);
  ASSERT_EQ(message_out.body, message_in.body);
...
TEST(message_test_message, can_serialize_message_view)
{
  std::default_random_engine random_engine;
  ...
  kd::message_header message_header_in;
  kd::message_body message_body_in;
  kd::message_view message_in{ .header = message_header_in,
                               .body = message_body_in };
  ASSERT_FALSE(ks::serialization::load(buffer, message_in));
